...
const fs = require('fs'); // Import fs for file system operations (like checking/creating directories)
const User = require('../models/User');

// --- Storage for Certificates ---
const certStorage = multer.diskStorage({
    destination: function (req, file, cb) {
        const dir = 'uploads/certificates/'; // Relative path from backend root
        // Create the directory if it doesn't exist
        if (!fs.existsSync(dir)) {
            fs.mkdirSync(dir, { recursive: true }); // recursive: true creates parent folders if they don't exist
        }
        cb(null, dir);
    },
...
exports.uploadCertificates = (req, res, next) => {
    uploadCert(req, res, async function (err) {
        ...
        try {
            ...
            req.files.forEach(file => { // Clean up uploaded files if DB update fails
...
const profileImageStorage = multer.diskStorage({
    destination: function (req, file, cb) {
        const dir = 'uploads/profile_images/'; // Relative path from backend root
        // Create the directory if it doesn't exist
        if (!fs.existsSync(dir)) {
...
const uploadProfilePic = multer({
    ...
    limits: { fileSize: 2 * 1024 * 1024 }, // 2 MB limit for profile pic
    fileFilter: (req, file, cb) => {
